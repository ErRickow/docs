...
Panduan ini mencakup pola tingkat produksi untuk menggunakan API Neosantara AI di seluruh endpoint Kompatibel OpenAI, Kompatibel Anthropic, dan API Responses. Fokusnya adalah pada kekamulan, performa, dan pemeliharaan.
...
async def postChatCompletion(session, payload):
...
async def main():
    async with httpx.AsyncClient() as session:
        ...
        for p in prompts:
            payload = p
            tasks.append(asyncio.create_task(postChatCompletion(session, payload)))
