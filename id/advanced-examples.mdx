...
description: "Pola siap produksi untuk menggunakan API Neosantara dengan banyak endpoint."
---

Panduan ini mencakup pola tingkat produksi untuk menggunakan API Neosantara di berbagai endpoint yang Kompatibel dengan OpenAI, Kompatibel dengan Anthropic, dan API Responses. Fokusnya adalah pada keandalan, performa, dan pemeliharaan.

## Keamanan & Manajemen Kunci

<Warning>
**Jangan pernah melakukan hard-code pada kunci API** atau mengeksposnya dalam kode sisi klien (browser, aplikasi seluler). Siapa pun yang memiliki akses ke kunci kamu dapat menggunakan kredit kamu.
</Warning>

### Praktik Terbaik

- **Hanya Sisi Server**: Selalu panggil API Neosantara dari backend kamu. Jika kamu harus menggunakannya di frontend, buat endpoint proxy di server kamu.
...
Setiap permintaan ke Neosantara mengembalikan `request_id` di header atau bodi respons. Selalu log ID ini bersama dengan log aplikasi kamu.
...
// Log ini ke sistem pemantauan kamu (Sentry, Datadog, dll.)
```
...
BASE = "https://api.neosantara.xyz"  # hindari duplikasi /v1 di ENDPOINT
API_KEY = "KUNCI_API_NEOSANTARA_KAMU"
ENDPOINT = f"{BASE}/v1/chat/completions"  # contoh; sesuaikan per endpoint
...
async def main():
    async with httpx.AsyncClient() as session:
        tasks = []
        prompts = [
            {"model": "garda-core", "messages": [{"role": "user", "content": "Halo"}]},
            {"model": "garda-core", "messages": [{"role": "user", "content": "Ceritakan sebuah cerita"}]}
        ]
        for p in prompts:
            payload = p
...
## Upaya Ulang & Toleransi Kesalahan (Retry & Fault Tolerance)
...
def call(payload):
    resp = requests.post("https://api.neosantara.xyz/v1/chat/completions", 
                         json=payload, 
                         headers={"Authorization": "Bearer <KUNCI_API>"})
...
async function callWithRetry(payload, retries = 3) {
  ...
  const headers = { 'Content-Type': 'application/json', 'Authorization': 'Bearer <KUNCI_API>' }
...
async function cachedCall(payload) {
  ...
  const res = await fetch('https://api.neosantara.xyz/v1/chat/completions', {
    method: 'POST', headers: { 'Authorization': 'Bearer <KUNCI_API>' , 'Content-Type': 'application/json' },
    body: JSON.stringify(payload)
  })
...
- Log peristiwa terstruktur untuk observabilitas (`request_id`, model, token yang digunakan)
- Kesadaran akan batas kecepatan per endpoint dan tier
...
<CardGroup cols={2}>
  ...
  <Card title="Batas Kecepatan" icon="gauge-high" href="/id/about/rate-limits">
    Tinjau panduan kuota dan pelambatan.
  </Card>
</CardGroup>