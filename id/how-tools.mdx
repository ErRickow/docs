...
Umumnya, model dengan kemampuan `function_calling` cocok untuk penggunaan tool. Untuk tool yang kompleks dan kueri yang ambigu, model seperti `garda-core`, `gpt-4o-mini`, `gemini-1.5-pro`, atau `command-r-plus` direkomendasikan karena mereka menangani pemilihan tool dan inferensi parameter dengan lebih kuat.
...
Tool tidak selalu harus berupa fungsi klien — kamu dapat menggunakan tool kapan pun kamu ingin model mengembalikan output JSON yang mengikuti skema yang disediakan. Misalnya, kamu mungkin menggunakan tool `record_summary` dengan skema tertentu untuk mengekstrak data terstruktur dari teks atau gambar.
