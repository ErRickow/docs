...
Panduan ini mendemonstrasikan cara membangun aplikasi Retrieval-Augmented Generation (RAG) yang skalabel dengan memanfaatkan **Cloudflare Vectorize** sebagai database vektor terkelola Kamu, yang terintegrasi dengan Neosantara untuk embedding dan pemanggilan model bahasa besar (LLM).
...
1. Menyiapkan klien Neosantara API dan kredensial API Cloudflare.
2. Membuat dan mengelola indeks Vectorize (v2).
3. Menggunakan model embedding Neosantara (`nusa-embedding-0001`) untuk melakukan vektorisasi dokumen Kamu.
...
6. Menggunakan model chat Neosantara (`nusantara-base`) untuk menghasilkan jawaban berdasarkan konteks yang diambil.
...
### Konfigurasi Kunci API dan Klien Kamu
...
NEOSANTARA_API_KEY = os.getenv("NEOSANTARA_API_KEY", "KUNCI_API_NEOSANTARA_KAMU")
...
def create_vectorize_index(index_name, dimension=768):
    ...
    # v2 menggunakan 'dimensions' bukan 'vector_size'
...
```python icon="python"
documents = [
  {"id": "doc1", "text": "Mengoperasikan Sistem Kontrol Iklim. Mobil Kamu memiliki sistem kontrol iklim yang memungkinkan Kamu menyesuaikan suhu dan aliran udara di dalam mobil."},
  {"id": "doc2", "text": "Kebijakan pengembalian dana kami memungkinkan pengembalian dalam waktu 30 hari setelah pembelian. Untuk memulai pengembalian dana, silakan hubungi tim dukungan kami di support@neosantara.xyz dengan nomor pesanan dan alasan pengembalian Kamu."},
...
def upsert_to_vectorize(documents_with_ids):
    ...
    for doc in documents_with_ids:
        ...
        # Format NDJSON: satu objek per baris
        vector_obj = {
            "id": doc['id'],
            "values": vector_data,
    ...
    # Gunakan Content-Type application/x-ndjson untuk v2 upsert
...
def query_vectorize(query_text, top_k=2):
    ...
    # v2 menggunakan returnMetadata sebagai string ("all", "indexed", "none")
...
Terakhir, buat prompt dengan konteks yang diambil dan kirimkan ke model chat Neosantara.
...
\n{context}\n---\n")
    ...
    # 3. Hasilkan jawaban menggunakan model chat Neosantara
...
if __name__ == "__main__":
    ...
    if "KUNCI_API_NEOSANTARA_KAMU" in NEOSANTARA_API_KEY or \
       ...
       "ID_AKUN_CLOUDFLARE_KAMU" in CLOUDFLARE_ACCOUNT_ID:
        print("⚠️ Silakan setel variabel lingkungan kunci API dan ID Akun Cloudflare Kamu.")
        exit()
