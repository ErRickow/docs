...
**Jendela Konteks (Context Window)** adalah jumlah informasi maksimal yang dapat diproses oleh model dalam satu interaksi. Ini mencakup **Input** (prompt Anda, instruksi, riwayat percakapan) dan **Output** (respon model).
...
Setiap model memiliki kapasitas berbeda untuk "mengingat" konteks. Memilih model yang tepat bergantung pada apakah Anda perlu memproses dokumen panjang (konteks tinggi) atau hanya interaksi cepat (konteks standar).
...
*   **Jendela Konteks (Input)**: Batas untuk prompt + riwayat percakapan Anda. Jika Anda mengirim 10.000 token teks ke `LuminAI` (batas 8k), permintaan akan gagal atau terpotong.
...
Ketika percakapan Anda tumbuh terlalu panjang, Anda mungkin mencapai batas. Neosantara menyediakan alat untuk menangani hal ini.

### Pemotongan Otomatis (Automatic Truncation)

Dalam **[Responses API](/api-reference/responses/create)**, Anda dapat menggunakan parameter `truncation`:
...
Anda selalu dapat memeriksa kolom `usage` dalam respon API untuk melihat seberapa dekat Anda dengan batas:
...
Jika kasus penggunaan Anda memerlukan pemrosesan jutaan token (misalnya, menganalisis seluruh buku atau basis kode), pertimbangkan untuk menggunakan solusi **RAG (Retrieval-Augmented Generation)** kami atau memecah konten Anda menjadi potongan-potongan (chunks).
