...
### 1. HTTP Headers
Untuk memastikan aliran data yang stabil, Neosantara menetapkan header berikut:
...
- `Connection: keep-alive`: Menjaga koneksi TCP tetap terbuka.

### 2. Sinyal Berhenti (Stop Signal)
Setelah pembuatan selesai, server mengirimkan pesan data akhir `[DONE]` atau event `response.completed`. Klien harus mendeteksi sinyal ini untuk menutup koneksi dan memproses hasil akhirnya.

---

## Implementasi Lengkap

Untuk contoh kode lengkap, parameter khusus SDK, dan praktik terbaik, silakan merujuk ke panduan integrasi berikut:

<CardGroup cols={3}>
  <Card title="Responses API" icon="bolt" href="/id/sdk/responses-api/streaming">
    Protokol berbasis event semantik yang direkomendasikan untuk proyek baru.
  </Card>
  <Card title="OpenAI Compatible" icon="openai" href="/id/sdk/openai-compat/chat-completions#streaming-chat-completion">
    Streaming berbasis chunk klasik untuk pengguna SDK OpenAI.
  </Card>
  <Card title="Anthropic Compatible" icon="bolt" href="/id/sdk/anthropic-compat/messages#streaming-messages">
    Dukungan native untuk format aliran pesan Anthropic.
  </Card>
</CardGroup>
