...
4. **Menghasilkan** jawaban yang akurat menggunakan `garda-core` atau `garda-agentic-mini`.
...
`);
  ...
  const response = await client.chat.completions.create({
    model: "garda-core", // Model tujuan umum
    messages: [
      {
        role: "system",
...
## Berintegrasi dengan Database Vektor
