...
description: '> Dapatkan respons terstruktur yang andal dari model AI kami dalam format JSON yang bersih, memudahkan integrasi langsung ke dalam aplikasi Anda.'
---

### Pengantar

Secara default, model bahasa menghasilkan teks biasa. Ini bagus untuk aplikasi seperti chatbot, tetapi menjadi sulit dikerjakan ketika Anda perlu mengekstrak detail spesifik dari respons secara terprogram.

Beberapa model di platform Nusantara AI memiliki kemampuan untuk merespons dalam format JSON terstruktur. Fitur ini memungkinkan Anda bekerja langsung dengan data output model dalam kode aplikasi Anda tanpa parsing teks yang rumit.

Untuk mengaktifkan fitur ini, Anda hanya perlu menambahkan parameter `response_format` ke permintaan API Anda.
...
Model yang berbeda mungkin mengembalikan JSON dalam format yang sedikit berbeda. Untuk memastikan aplikasi Anda tangguh, yang terbaik adalah mem-parsing respons dengan fungsi pembantu yang dapat menangani variasi ini. Beberapa model membungkus JSON dalam blok markdown (misalnya, ` ```json ... ``` `), sementara yang lain mungkin menumpuk string JSON di dalam objek JSON lain.

Contoh berikut menyertakan fungsi parsing yang tangguh.

<Info>
Anda **harus** menginstruksikan model untuk merespons hanya dalam format JSON dalam system prompt atau pesan pengguna Anda, selain menyertakan parameter `response_format`.
</Info>
...
# Anda perlu menginstal: pip install openai
import json
import re
    ...
    try:
        response = client.chat.completions.create(
            ...
            messages=[
                {
                    "role": "system",
                    "content": "Anda adalah asisten yang membantu menyediakan resep dalam format JSON terstruktur. Pastikan respons HANYA objek JSON."
                },
                {
                    "role": "user",
                    "content": "Saya butuh resep nasi goreng. Saya punya nasi, telur, bawang merah, bawang putih, dan kecap. Masaknya sekitar 15 menit."
...
Setelah menjalankan salah satu contoh di atas, variabel `output` Anda akan berisi objek JSON yang bersih dan terurai:
...
Dengan menggunakan fungsi parsing yang tangguh di sisi klien, Anda dapat menangani data terstruktur dari model apa pun di platform Nusantara AI dengan andal.
