...
| **State Management** | ✅ Supported | Managed via `store: true` and `previous_response_id`. See [Conversation Management](/api-reference/responses#conversation-management). |
| **Input Format** | Flexible | Supports `string` or `array` of messages. See [Input Formats](/api-reference/responses#input-formats). |
| **Tool Use** | ✅ Enhanced | Supports native tool definitions and execution. See [Tool Calling](/api-reference/responses#function-tool-calling). |
| **Streaming** | ✅ Supported | Use `stream: true` for SSE. See [Streaming](/api-reference/responses#streaming). |
