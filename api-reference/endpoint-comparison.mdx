...
| **Tool Calling** | ✅ Standard format | ✅ Tool schema | ✅ Enhanced |
...
<Tip>
  **Recommendation**: Use Responses API for new applications, OpenAI Compatible for existing OpenAI-based apps, and Anthropic Compatible for Anthropic ecosystem integration.
</Tip>
...
| Feature | OpenAI Compatible | Anthropic Compatible | Responses API |
|---------|-------------------|---------------------|----------------|
| Conversation state management | ❌ | ❌ | ✅ `store: true` |
...
| Debugging ease | 🟢 Standard | 🟢 Standard | 🟢 Enhanced |
| Error handling | 🟢 Standard | 🟢 Standard | 🟢 Enhanced |
...
response = client.responses.create(
    ...
    store=True  # Automatic conversation management
)
```
...
response = client.responses.create(
    ...
    tools=[...],
    store=True,  # Automatic state management
    stream=True
)
...
| Connection reuse | 🟢 Standard | 🟢 Standard | 🟢 Enhanced |
...
| Responses API | Per-model, per-key | Enhanced with store |
...
def chat_with_history(conversation_id, new_message):
    response = client.responses.create(
        model="nusantara-base",
        input=new_message,
...
<CardGroup cols={2}>
  <Card title="SDK Documentation" icon="book" href="/api-reference/sdk/">
