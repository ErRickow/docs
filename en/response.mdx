...
When you use an official OpenAI SDK like [`openai-python`](https://pypi.org/project/openai/) or [`openai-node`](https://www.npmjs.com/package/openai) with Neosantara's `base_url`, the library maps its methods to our API endpoints. While both [`v1/responses`](/api-reference/responses/create) and [`v1/chat/completions`](/api-reference/chat/completions/create) can hold a conversation, how you interact with them through the SDK is fundamentally different.
...
- **Responses**: The SDK provides a convenient and safe helper property called `output_text` that automatically filters and concatenates text from all message parts. This is the recommended way to access the AI's reply unless you need to manually inspect non-text content (like tool calls) in the raw `output` array.
...
# The recommended and safe way
print(response.output_text)

# For advanced use cases (e.g., iterating raw output parts)
text_parts = []
...
completion = client.chat.completions.create(model="garda-core", messages=messages) # [!code --]
...
next_completion = client.chat.completions.create(model="garda-core", messages=messages) # [!code --]
...
import openai # [!code --]
client = openai.OpenAI(base_url="https://api.neosantara.xyz/v1") # [!code --]
