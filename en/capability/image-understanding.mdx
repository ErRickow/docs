...
*   **Permanent Storage**: If you require images to persist across multiple sessions (e.g., for Batch API), use the [Files API](/en/files-overview) to upload and retrieve a `file_id`.
...
<CardGroup cols={3}>
  <Card title="Responses API" icon="bolt" href="/en/sdk/responses-api/image-input">
    Using the `image` part type in the modern, stateful API.
  </Card>
  <Card title="OpenAI Compatible" icon="openai" href="/en/sdk/openai-compat/chat-completions#image-attachments">
    Standard `image_url` implementation for the OpenAI SDK.
  </Card>
  <Card title="Anthropic Compatible" icon="bolt" href="/en/sdk/anthropic-compat/file-attachments">
    Using the native Anthropic base64 image block format.
  </Card>
</CardGroup>
