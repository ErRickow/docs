...
4. **Generates** an accurate answer using `garda-core` or `garda-agentic-mini`.
...
`);
  ...
  const response = await client.chat.completions.create({
    model: "garda-core", // General purpose model
    messages: [
      {
        role: "system",
...
// const pc = new Pinecone({ apiKey: 'YOUR_NEOSANTARA_API_KEY' });
