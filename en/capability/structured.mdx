...
| **Neosantara Flagship** | `archipelago-core-70b`, `garda-core`, `garda-agentic-mini`, `sea-lion-v4-27b-it` |
| **Llama & Specialized** | `llama-3.3-nemotron-super-49b-v1.5`, `llama-3.3-70b-instruct`, `llama-3.2-11b`, `qwen3-32b`, `grok-4.1-fast-non-reasoning`, `grok-code-fast`, `granite-3-8b-instruct`, `holo2-30b`, `hermes-2-pro-mistral-7b` |
...
For complete code examples, schema definitions, and parsing patterns, please refer to:

<CardGroup cols={3}>
  <Card title="Responses API" icon="bolt" href="/en/sdk/responses-api/index">
    Recommended for strict, automated schema enforcement.
  </Card>
  <Card title="OpenAI Compatible" icon="openai" href="/en/sdk/openai-compat/structured-outputs">
    Standard JSON mode and schema support using the OpenAI SDK.
  </Card>
  <Card title="Anthropic Compatible" icon="bolt" href="/en/sdk/anthropic-compat/tool-calls">
    Extracting structured data via tool definitions in the Anthropic format.
  </Card>
</CardGroup>
...
Neosantara employs several strategies to ensure your structured data is parseable and accurate.

### Instruction Injection
For models that do not natively support JSON mode, Neosantara's **Standardizer** automatically appends system-level instructions (e.g., *"Respond ONLY with a valid JSON object"*) to the end of your prompt. This significantly increases consistency across older or smaller open-source models.

### Markdown Stripping
Language models often wrap JSON in markdown code blocks (e.g., \` \` \`json ... \` \` \`). Neosantara's backend includes an **Extraction Filter** that automatically detects and removes these backticks before returning the string to your application, preventing common `JSON.parse` errors.

### Schema Enforcement
While **JSON Mode** is a "soft constraint" (the model tries its best), the **Responses API** utilizes internal schema-validation logic. It cross-references the model's output against your provided JSON Schema and can even re-prompt the model if the first attempt fails validation (depending on the internal provider's retry logic).
