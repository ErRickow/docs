...
* **`image`** (string, optional): The base64-encoded input image for image-to-image (variation), inpainting, or outpainting tasks.
* **`mask_image`** (string, optional): The base64-encoded mask image for inpainting or outpainting.
* **`outpainting_mode`** (string, optional): The mode for outpainting tasks (e.g., `"DEFAULT"`, `"PRECISE"`).
...
* **`steps`** (integer, optional, default: `1`): The number of diffusion steps for image generation.
* **`response_format`** (string, optional, default: `"url"`): The format in which the generated images are returned (`"url"` or `"b64_json"`).
...
Modify specific areas of an existing image. Provide the original image in the `image` parameter and a binary mask in the `mask_image` parameter (where white areas represent the parts to be modified).

### Outpainting

Extend an image beyond its original borders. Provide the original image in `image` and set `outpainting_mode` to `"DEFAULT"` or `"PRECISE"`.

## Supported Image Generation Models

The NeosantaraAI API supports the following image generation models:

* **`titan-image-g1`**: Amazon Titan Image Generator G1 v2. Extremely cost-effective and versatile. Supports text-to-image, variations, inpainting, and outpainting. Includes invisible watermarking for safety.
* **`neosantara-gen-2045`**: Our high-quality legacy image generation model. Recommended for creative and artistic outputs.
...
// Replace "YOUR_NEOSANTARA_API_KEY" with your actual NeosantaraAI API Key
