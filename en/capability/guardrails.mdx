...
Neosantara API provides built-in **Guardrails** to help you comply with Indonesian data privacy laws ([**UU PDP No. 27/2022**](https://pdp.kominfo.go.id/)). When enabled for a request, this feature detects and redacts sensitive Personally Identifiable Information (PII) from your prompts before they are sent to the upstream AI providers (like OpenAI, Anthropic, or Grok).

## How It Works

*   **Request-Level Protection**: For users on the Basic tier and above, Guardrails can be enabled per request with the `X-Guard: on` header.
...
Paid users can enable Guardrails using the `X-Guard` header. Requests without this header are sent without server-side PII redaction.

| Value | Description |
| :--- | :--- |
| `on` | Enables PII redaction for the request. |
| `off` | Leaves PII redaction disabled. Data will be sent raw to the provider. |
...
Neosantara AI is fully compatible with [**openai-guardrails-js**](https://github.com/openai/openai-guardrails-js) and other similar client-side libraries. By combining these tools, you get **Double Guarding**: client-side validation for instant feedback and Neosantara's server-side Indonesian PII protection.

To integrate, simply point the `baseURL` to Neosantara and use your `nsk_` key.
