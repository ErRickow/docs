...
For standard models that don't have native reasoning (e.g., `garda-core`, `archipelago-core-70b`), Neosantara uses **Instruction Injection**. When reasoning is enabled, the system automatically prepends complex logical instructions to the system prompt, forcing the model to break down its process into steps.
