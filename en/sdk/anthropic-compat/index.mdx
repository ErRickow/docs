...
description: "Neosantara AI Gateway provides Anthropic-compatible API endpoints, allowing you to use official Anthropic SDKs and tools like Claude Code."
---
...
For more on using Neosantara with Claude Code, see the [AI Coding Tools guide](/en/guides/ai-coding-tools).
...
Neosantara supports the following Anthropic-compatible endpoint:
...
*   [Advanced Features](/en/sdk/anthropic-compat/advanced) - Extended thinking.
...
*   Monitor traffic and spend in your [usage overview](/en/about/billing-pricing).
*   Use models like `claude-4.5-sonnet` or `grok-code-fast` for coding tasks.

1.  ### Configure environment variables
    
    Configure Claude Code to use Neosantara AI Gateway by setting these [environment variables](https://code.claude.com/docs/en/settings#environment-variables):
    ...
    ```bash
    alias claude-nai='ANTHROPIC_BASE_URL="https://api.neosantara.xyz/anthropic" ANTHROPIC_AUTH_TOKEN="your-api-key-here" ANTHROPIC_API_KEY="" claude'
    ```
    ...
    For more flexibility, create a wrapper script at `~/bin/claude-nai`:
    
    ```bash
    #!/usr/bin/env bash
    # Routes Claude Code through Neosantara
...
2.  ### Run Claude Code
    ...
    Run `claude-nai` to start Claude Code with Neosantara:
...
Point your client to the Neosantara AI Gateway base URL and use your [API key](/en/authentication) for authentication.
