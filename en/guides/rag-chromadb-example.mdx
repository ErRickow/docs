...
description: "Build a Retrieval-Augmented Generation (RAG) application using Neosantara and ChromaDB."
---

This guide demonstrates how to build a complete Retrieval-Augmented Generation (RAG) application using Neosantara for embeddings and large language model (LLM) calls, integrated with [ChromaDB](https://www.trychroma.com/) as your vector database.

## Overview

You will learn to:

1. Set up your Neosantara API client.
2. Install and initialize ChromaDB.
3. Use Neosantara's embedding model (`nusa-embedding-0001`) to vectorize your documents.
4. Store these embeddings in ChromaDB.
5. Retrieve relevant documents from ChromaDB based on a user query.
6. Use Neosantara's chat model (`garda-core`) to generate a grounded answer using the retrieved context.
...
### Configure your Neosantara API Key

You'll need a Neosantara API Key. If you don't have one, get it from your [Neosantara Dashboard](https://app.neosantara.xyz/dashboard).

Set your API key and Neosantara base URL as environment variables or directly in your script:

```python icon="python"
import os
from openai import OpenAI

# Configure your Neosantara API key and base URL
NEOSANTARA_API_KEY = os.getenv("NEOSANTARA_API_KEY", "YOUR_NEOSANTARA_API_KEY_HERE")
NEOSANTARA_BASE_URL = os.getenv("NEOSANTARA_BASE_URL", "https://api.neosantara.xyz/v1")

# Initialize the OpenAI client pointing to Neosantara
client = OpenAI(
...
ChromaDB allows you to define a custom embedding function. We'll wrap the Neosantara embedding API call within this function.
...
class NeosantaraEmbeddingFunction(EmbeddingFunction):
  def __call__(self, input: Documents) -> Embeddings:
    # Neosantara's embedding API expects a list of strings
...
Finally, construct a prompt with the retrieved passage and send it to a Neosantara chat model to generate a response.
...
def generate_answer(query, db):
  ...
  # 3. Call the Neosantara chat model
...
if __name__ == "__main__":
  # Ensure API Key and Base URL are set in environment or replace "YOUR_NEOSANTARA_API_KEY_HERE"
  if "YOUR_NEOSANTARA_API_KEY_HERE" in NEOSANTARA_API_KEY:
      print("⚠️ Please set your NEOSANTARA_API_KEY environment variable or replace 'YOUR_NEOSANTARA_API_KEY_HERE' in the script.")
