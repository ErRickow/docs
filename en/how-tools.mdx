...
Generally, models with `function_calling` capabilities are suitable for tool use. For complex tools and ambiguous queries, models like `garda-core`, `gpt-4o-mini`, `gemini-1.5-pro`, or `command-r-plus` are recommended as they handle tool selection and parameter inference more robustly.
